...
class Instagram {
    ...
    public static function getMediaByUserID($user = null, $count = 16, $assoc = false, $comment_count = false)
    {
        ...
        $parameters = urlencode("ig_user($user) { media.first($count) {   count,   nodes {    location { lat, lng },    caption,     code,     $comments,     date,     dimensions {       height,       width     },     display_src,     id,     is_video,     likes {       count     },     owner {       id,       username,       full_name,       profile_pic_url,     biography     },     thumbnail_src,     video_views,     video_url   },   page_info }  }");
        $url = "https://www.instagram.com/query/?q=$parameters&ref=tags%3A%3Ashow";
        $media = json_decode(file_get_contents($url),($assoc || $assoc == "array"));
        if($assoc == "array")
            $media = $media["media"]["nodes"];
    ...
    public static function getAllMediaByUserID($user = null)
    {
        if ( empty($user) || !(is_string($user) || is_int($user)) )
        {
            return false;
        }
        $parameters = urlencode("ig_user($user) { media.first(1) { count } }");
        $url = "https://www.instagram.com/query/?q=$parameters&ref=tags%3A%3Ashow";
        $media = json_decode(file_get_contents($url), true);
        if(array_key_exists("media", $media) && $media["media"]["count"] > 0) {
            return Instagram::getMediaByUserID($user, $media["media"]["count"], "array");
        }
        return false;
    }
